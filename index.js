  ...
  databaseURL: "https://gamebuzzuer-default-rtdb.firebaseio.com", // 🔥 أضف هذا
  ...
  appId: "1:952611711946:web:881006b2fa17693307bd33"
    ...
    const TEAM_COLORS = ["team-red", "team-blue", "team-green", "team-purple", "team-slate"];
    ...
    function sessionRef(code) { return ref(db, `sessions/${code}`); }
    function presenceRef(code) { return ref(db, `sessions/${code}/presence/${local.deviceId}`); }
    ...
    function normalizeSession(raw, code) {
      const teams = Array.isArray(raw?.teams) && raw.teams.length ? raw.teams : defaultTeams();
      return {
        code,
    ...
    function canBuzz(session) {
      return !session.locked && session.winnerTeamId === null && session.timeLeft > 0;
    }
    ...
    function renderSession(session) {
      ...
      els.timerStatusBadge.className = `state-badge ${session.timerRunning ? "green" : ""}`.trim();
      els.toggleTimerBtn.textContent = session.timerRunning ? "إيقاف مؤقت" : "تشغيل المؤقت";
    ...
    function renderTeamSelect(session) {
      ...
      const selected = session.teams.find(t => t.id === Number(els.selectedTeam.value));
    ...
    async function subscribeToSession(code) {
      ...
      switchTab(new URLSearchParams(location.search).get("session") ? "device" : "host");
    }
    ...
    async function toggleTimer() {
      ...
      if (session.timerRunning) {
        await updateSessionPatch({ timerRunning: false, roundEndsAt: null, hostUpdatedAt: Date.now() });
      } else {
        await updateSessionPatch({
          timerRunning: true,
          roundStartedAt: Date.now(),
    ...
    async function toggleLock() {
      const session = await readCurrentSession();
      await updateSessionPatch({ locked: !session.locked, hostUpdatedAt: Date.now() });
    }

    async function clearWinner() {
      await updateSessionPatch({ winnerTeamId: null, locked: false, hostUpdatedAt: Date.now() });
    }
    ...
    async function addPoint() {
      ...
      const teams = session.teams.map(team => team.id === session.winnerTeamId ? { ...team, points: (team.points || 0) + 1 } : team);
    ...
    async function addTeam() {
      ...
      const colorClass = TEAM_COLORS[session.teams.length % TEAM_COLORS.length] || "team-slate";
    ...
    async function updateTeamName(teamId, name) {
      const session = await readCurrentSession();
      const teams = session.teams.map(team => team.id === teamId ? { ...team, name } : team);
      await updateSessionPatch({ teams, hostUpdatedAt: Date.now() });
    }
    ...
    async function joinByCode() {
      try {
        const code = String(els.joinCodeInput.value || "").trim().toUpperCase();
        if (!code) {
    ...
    function bindEvents() {
      ...
      els.createSessionBtn.addEventListener("click", () => createOrLoadSession(randomCode()));
      els.copyCodeBtn.addEventListener("click", () => copyText(local.currentSessionCode, "تم نسخ الكود"));
      els.copyJoinBtn.addEventListener("click", () => copyText(getJoinUrl(), "تم نسخ رابط الدخول"));
      ...
      els.newTeamName.addEventListener("keydown", (e) => { if (e.key === "Enter") addTeam(); });
      ...
      els.deviceBuzzBtn.addEventListener("click", () => claimBuzz(Number(els.selectedTeam.value)));
      els.joinByCodeBtn.addEventListener("click", joinByCode);
      els.joinCodeInput.addEventListener("keydown", (e) => { if (e.key === "Enter") joinByCode(); });
